...
async function ExtractFields(record: ScrapedPageRecord, content: string, model = 'openai/gpt-oss-20b') {
  ...
  console.log(chatCompletion.getContent());
  ...
  if (existing_record) {
    ...
    console.log(`Updated apartment posting for ${existing_record.type} ${existing_record.area}m2 selling for ${existing_record.price}`);
  }
  else {
  ...
  return chatCompletion.getContent();
}

async function ProcessRecords(records: ScrapedPageRecord[]) {
  let count = 0;
  for (const record of records) {
    console.log(`[ApartmentPostingService] Extracting fields from ${path.basename(record.URL)}`);
...
async function ExtractAllFields(message: MessageWrapper) {
  ...
  const new_records = await filteredEntries.filter(async (entry) => !await ApartmentPostingRepository.GetWithSource(entry.URL));

  console.log(`[ApartmentPostingService] Began extracting fields from`, filteredEntries.length, " queued pages.");

  count += await ProcessRecords(new_records);

  const existingrecords = await filteredEntries.filter(async (entry) => await ApartmentPostingRepository.GetWithSource(entry.URL));
