...
import { McKinseyInsights, ScrapeSource } from "./ScrapeSource";
import { IndustryFact, IndustryFactRepository } from "./IndustryFact";
...
async function RunFullScraping(message: MessageWrapper, source: ScrapeSource) {
  ...
  try {
    ...
    while (URLsQueue.length) {
      ...
      await Sleep(1000 + getRandomInt(25000));
...
async function ExtractFields(record: ScrapedPageRecord, content: string, model = 'openai/gpt-oss-20b') {
  ...
  if (!content.toLowerCase().includes("report") && !content.toLowerCase().includes("article") && !content.toLowerCase().includes("blog post") && !content.toLowerCase().includes("charts") && !content.toLowerCase().includes("case study"))
    return;
  ...
  const messages = [
    { role: 'system', content: "Extract key factoids from the article. Each factoid should include a number, be it a number, a percent, or a dollar value. Ignore reading time and subscription offers." },
    ...
    response_format: {
      ...
      "json_schema": {
        ...
        "schema": {
          ...
          "properties": {
            "facts": {                      // the array is now a property
              "type": "array",
              "items": {
                "type": "object",
                "properties": {
                  "topic": { "type": "string", "description": "Broad categorization..." },
                  "fact": { "type": "string", "description": "The industry fact text." },
                  "date": { "type": "string", "description": "YYYY-MM-DD format" }
                },
                "required": ["topic", "fact", "date"],
  ...
  for (const choice of chatCompletion.choices) {
    const fields = JSON.parse(choice.message.content || '{"facts": []}')?.facts as Array<any> || [];
    console.log(choice.message.content);
    console.log(fields);

    if (!fields.length) {
      // Must be a mistake why an article outputted no facts
      record.LAST_FETCHED = 0;
      await ScrapedPageRecordRepository.Update(record);
    }

    //US6AC3 Scraper inserts new postings it found in the DB
    //US6AC4 Scraper updates postings coming from the same source
    const existing_record = await IndustryFactRepository.GetWithSource(url);
    if (existing_record.length) {
      ...
      for (const elem of fields) {
        ...
        fact.fact = elem?.fact;
        fact.source = url;
        IndustryFactRepository.Insert(fact);
      }
    }
...
export async function ProcessScraper(message: MessageWrapper) {
  if (message.checkRegex(/\/scrape/)) {
    RunFullScraping(message, McKinseyInsights);
    return true;
  }
